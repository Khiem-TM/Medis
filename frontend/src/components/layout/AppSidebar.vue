...
<template>
  <aside
    :class="[
      'fixed inset-y-0 left-0 z-40 bg-white border-r border-[#E5E7EB] flex flex-col transition-all duration-200',
      uiStore.sidebarOpen ? 'w-60' : 'w-16',
    ]"
  >
    <!-- Logo -->
    <div class="flex items-center gap-3 px-4 py-5 border-b border-[#E5E7EB]">
      ...
      <span v-if="uiStore.sidebarOpen" class="text-lg font-bold text-[#111827] truncate">Medis</span>
    </div>
    ...
    <div class="border-t border-[#E5E7EB] p-3">
      <RouterLink
        to="/profile"
        class="flex items-center gap-3 p-2 rounded-lg hover:bg-[#F9FAFB] transition-colors"
      >
